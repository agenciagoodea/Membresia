...
const Switch = ({ active, onChange }: { active: boolean; onChange: () => void }) => (
  <div 
    ...
    className={`w-14 h-8 rounded-full p-1 cursor-pointer transition-all duration-300 relative border ${
      active ? 'bg-blue-600 border-blue-400/50 shadow-[0_0_15px_rgba(37,99,235,0.3)]' : 'bg-zinc-950 border-white/10'
    }`}
  >
    <div className={`w-6 h-6 bg-white rounded-full shadow-lg transition-all duration-300 transform ${
...
const PrayerForm: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const existing = JSON.parse(localStorage.getItem('ecclesia_prayer_requests') || '[]');
    localStorage.setItem('ecclesia_prayer_requests', JSON.stringify([...existing, newRequest]));
    setSubmitted(true);
  };
      ...
      <div className="w-full max-w-2xl relative z-10">
        ...
        <form onSubmit={handleSubmit} className="bg-zinc-900 p-10 md:p-16 rounded-[4rem] border border-white/5 shadow-[0_50px_100px_rgba(0,0,0,0.5)] space-y-12">
            ...
            <div onClick={() => fileInputRef.current?.click()} className="group border-2 border-dashed border-white/10 rounded-[2.5rem] p-12 flex flex-col items-center justify-center cursor-pointer hover:border-blue-500/50 hover:bg-blue-500/5 transition-all bg-zinc-950 min-h-[200px] relative overflow-hidden shadow-inner">
              ...
              {formData.photo ? (
                <div className="relative">
                   <img src={formData.photo} className="max-h-48 rounded-3xl shadow-2xl ring-4 ring-zinc-900" alt="" />
                   <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-3xl"><X className="text-white" size={32} /></div>
                </div>
              ) : (
          ...
          <div className="space-y-8">
            ...
            <button type="submit" className="w-full py-7 bg-white text-zinc-950 rounded-[2.5rem] text-sm font-black uppercase tracking-[0.2em] shadow-2xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-4 group">
