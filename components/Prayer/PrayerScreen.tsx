...
const PrayerScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Carregar pedidos do localStorage + mocks para o telão
    const stored = JSON.parse(localStorage.getItem('ecclesia_prayer_requests') || '[]');
    const all = [...MOCK_PRAYER_REQUESTS, ...stored];
    ...
    <div className="h-screen w-screen bg-slate-950 overflow-hidden relative flex items-center justify-center font-sans selection:bg-rose-500">
      ...
      <div key={current.id} className="relative z-10 w-full max-w-[1600px] px-24 py-12 flex flex-col items-center animate-in fade-in zoom-in-95 duration-1000">
        ...
        <div className="flex flex-col xl:flex-row items-center gap-24 xl:gap-32 w-full">
          {/* Photo Container - Intelligent format adaptation */}
          <div className="shrink-0 relative group">
            ...
            <div className="relative bg-slate-900 rounded-[5rem] border-[16px] border-white/5 shadow-2xl overflow-hidden flex items-center justify-center w-[500px] h-[500px] xl:w-[600px] xl:h-[600px]">
              {current.photo ? (
                <img 
                  src={current.photo} 
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-slate-800">
                  ...
                  <p className="text-xl font-black uppercase tracking-[0.5em] mt-8 opacity-20">Clamor Silencioso</p>
                </div>
              )}
            </div>

            {/* Status Floating Badge */}
            <div className={`absolute -bottom-8 -right-8 w-40 h-40 rounded-[2.5rem] flex items-center justify-center shadow-3xl border-[12px] border-slate-950 transition-all duration-700 scale-110 ${
              current.status === PrayerStatus.IN_PRAYER ? 'bg-emerald-500' : 'bg-rose-600'
            }`}>
          ...
          <div className="flex-1 text-center xl:text-left space-y-12">
            <div className="space-y-4">
               <p className={`text-3xl font-black uppercase tracking-[0.4em] flex items-center justify-center xl:justify-start gap-4 ${
                 current.status === PrayerStatus.IN_PRAYER ? 'text-emerald-400' : 'text-rose-500'
               }`}>
               ...
               <h2 className="text-8xl xl:text-9xl font-black text-white tracking-tighter leading-[0.9] drop-shadow-2xl">
                 {displayName}
               </h2>
            </div>
        ...
        <div className="mt-32 w-full flex items-center justify-between px-16 py-8 bg-white/5 rounded-[4rem] border border-white/10 backdrop-blur-2xl">
          ...
          <div className="flex items-center gap-12">
             ...
             <div className="w-24 h-24 rounded-3xl bg-white flex items-center justify-center shadow-2xl p-2 group overflow-hidden">
                <div className="w-full h-full bg-slate-900 rounded-2xl flex items-center justify-center text-white font-black text-[8px] text-center">
                   ESCANEIE<br/>AQUI
                </div>
             </div>
          </div>
